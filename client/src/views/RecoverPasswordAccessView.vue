        ...
        <v-form ref="recoverForm" v-model="valid" lazy-validation>
          <v-row>
            ...
            <v-col class="d-flex" cols="12" sm="6" xsm="12" align-end>
              <v-btn large block @click="main_menu()">Retour</v-btn>
...
export default {
  ...
  data: () => ({
    ...
    rules: {
      ...
      email: (value) =>
        /.+@.+\..+/.test(value) || "L'adresse courriel doit être valide.",
    },
  }),
  methods: {
    async main_menu() {
    ...
    async validate() {
      if (this.$refs.recoverForm.validate()) {
        ...
        if (!email_send) {
          ...
          alert.innerHTML =
            "Un courriel a été envoyé à l'adresse précisée précédemment!";
        }
